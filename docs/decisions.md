...
\- Post-v1 (2026-07-15): two review-view fixes. (a) Update-thread recency labels were tied to the review-window highlight, so when the window excluded the newest update, an in-window older update got labelled "Latest update" and the newer out-of-window one "Previous update". Decoupled: the single newest update (updates[0]) is always "Latest update", older ones "Previous update"; the window only drives the amber highlight, labelled "Updated in this period" (`TaskUpdateThread.jsx`). (b) Default milestone expansion now follows the date selector: period "All" expands ALL milestones + the project group (full view); a window collapses to review mode and auto-expands only milestones with an in-period update. Applied once per period change (keyed) so manual expand/collapse survives background data refreshes. Client-only.
